...
> ⚠️ **ATTENTION: FIRST LAUNCH INITIALIZATION** > On the very first startup, the API service will automatically extract and initialize heavy native hardware-acceleration libraries (e.g., local CUDA binaries) into your mounted volume. This one-time user-space extraction expands approximately 500MB of data and can take **1 to 3 minutes** depending on your disk speed. 
> 
> During this phase, the Management UI will display an **"Initializing runtime... ⚙️"** status indicator. Please be patient and do not restart the containers until the extraction completes and the status changes to **"Ready"**.
